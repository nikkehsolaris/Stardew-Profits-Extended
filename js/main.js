...
function fetchCrops() {
	...
	for (var i = 0; i < season.crops.length; i++) {
	    if ((options.seeds.pierre && season.crops[i].seeds.pierre != 0) ||
	    	(options.seeds.joja && season.crops[i].seeds.joja != 0) || (options.seeds.special && season.crops[i].seeds.special != 0)) {
...
function renderGraph() {
	...
	svg.attr("width", barOffsetX + barPadding * 2 + (barWidth + barPadding) * cropList.length + 30);
	d3.select(".graph").attr("width", barOffsetX + barPadding * 2 + (barWidth + barPadding) * cropList.length + 30);
